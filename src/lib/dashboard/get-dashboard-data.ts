...
export type DashboardItemKind = "bill" | "chore" | "maintenance" | "utility";
...
export async function getDashboardData(): Promise<DashboardData> {
  ...
  function place(item: DashboardItem) {
  ...
  const [choreCount, maintenanceCount, recentActivity] = await Promise.all([
  ...
  for (const key of Object.keys(buckets) as (keyof DashboardBuckets)[]) {
  ...
  return {
    ...
    buckets,
    overview: {
      utilities: householdUtilities.length,
